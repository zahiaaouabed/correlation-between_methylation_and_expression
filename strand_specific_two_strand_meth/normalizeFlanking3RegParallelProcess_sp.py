...
def readProfile(i, profile, achr, tss, tts, bin_size, nb_partition) :
    ...
    output_filename = "3met/"+tcontext+"/s"+str(i)+".met_chr"+achr+"_"+str(tss)+"_"+str(tts)+".gene."+tcontext+".3.flanking.met"
        ...
        for row in spamreader:
            if (len(row) >= 8) :
                ...
                for x in range(0, nb_partition):
                    ...
                    if ((position > tts - 5000 + x * bin_size) and (position <= tts - 5000 + (x+1) * bin_size)) :

                        if ((cov >= 3) and (context == tcontext)) : 
                            counts[x] = counts[x] + 1
     ...
     for row in spamreader:
         ...
         for i in subjects:
             ...
             if (os.path.exists(profile)) :
                  bin_size = 5000/nb_partition;
                  t = mp.Process(name = i+"_"+row[0]+"_"+row[1]+"_"+row[2], target = readProfile, args = (i, profile, row[0], int(row[1])-5000, int(row[2])+5000, bin_size, nb_partition))
                  processes.append(t)
